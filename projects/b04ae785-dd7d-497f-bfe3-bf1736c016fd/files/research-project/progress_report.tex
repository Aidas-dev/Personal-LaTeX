...
Word count: 737  \hfill{} Dr. Aidas Masiliūnas and Dr. Laura Galdikienė 
...
Most studies examine single democratically chosen institutions \citep{Dannenberg2020Choice, gurerk2006} or multiple imposed institutions \citep{tao2016}; we compare multiple endogenously implemented institutional types. Our research provides systematic comparison of individual vs. collective and punishment vs. reward mechanisms within a unified experimental framework.

\section{Literature Review}

The standard public goods game, developed by \citet{isaac1988} and replicated multiple times, reliably demonstrates the cooperation decay phenomenon where contributions decrease over repeated rounds despite the social optimum requiring full cooperation.

Research on institutional interventions has shown that punishment mechanisms can sustain cooperation \citep{fehr2000, Dannenberg2020Choice}, though they may reduce overall efficiency due to the costs of punishment. Reward systems have also proven effective, though potentially less efficient than punishment \citep{tao2016}.
...
We employ a within-subjects design with 25 total rounds conducted using oTree software. Each group consists of 5 participants (24 groups in main study, 120 total participants), with group composition remaining stable throughout the experiment. The participants will be Vilnius university students, no economics students.
...
After each round participants are shown their contribution, profit and the group averages; every 5 rounds they are shown 2 informative graphs.\footnote{These graphs display the average profit and average contribution of all participants, calculated specifically for the preceding 5-round period, this is shown before voting begins.}
...
\textbf{Variable definitions:} Endowment $e = 10$, contribution $c_i \in [0,10]$, MPCR $m = 0.4$, group size $N = 5$.
...
\textit{Assumptions:} $\alpha=0.05$, $\beta=0.20$, $\rho=0.5$, $\sigma=4.0$.
  
\textbf{1. Group-level analysis (voting outcomes):}
\begin{itemize}
    \item With 24 independent group observations
    \item 80\% power to detect: \textbf{$d \approx 0.82$} (large effect) in between-group comparisons
\end{itemize}

\textbf{2. Individual-level analysis (contributions):}
\begin{itemize}
    \item With 120 participants in within-subjects design ($\rho = 0.5$)
    \item 80\% power to detect: \textbf{$d \approx 0.26$} (small-to-medium effect)
    \item For medium effects ($d = 0.5$): Power $> 98\%$
\end{itemize}

\textbf{3. Mixed-effects model considerations:}
With 24 groups and 120 individuals:
\begin{itemize}
    \item Adequate for estimating group-level random intercepts
    \item May be limited for group-by-treatment interactions
    \item Sufficient for main effects of institutions on contributions
\end{itemize}

\textbf{Summary:} Our sample provides:
\begin{itemize}
    \item High power for \textit{within-individual} comparison of contributions across institutions
    \item Adequate power for detecting \textit{large effects} in group-level voting patterns
\end{itemize}
...
\begin{axis}[
    ...
    ymin=0, ymax=10,
    legend style={
        at={(0.5,-0.15)}, % Position below the plot
        anchor=north,
        legend columns=2, % Two columns side by side
        cells={anchor=west},
        draw=none % Remove box around legend
    },
    grid=both,
    width=0.8\textwidth,
    height=0.6\textwidth
...
\addlegendentry{With Institutions (Average of All Institutions)}
...
\label{fig:expected-contributions}
\end{figure}
...
\begin{itemize}
\item H1: Regression on first voting choice
...
\item Interval between voting might be to short
...
\item Longer periods between voting
\end{itemize}
